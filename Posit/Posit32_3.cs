...
namespace Lombiq.Arithmetics
{
	public readonly struct Posit32_3 : IComparable, IConvertible, IFormattable, IEquatable<Posit32_3>, IComparable<Posit32_3>
	{
		...
		public uint GetExponentValue()
		{
			...
			exponentMask = (uint)((exponentMask >> (int)FractionSize())
							<< (Size - exponentSize)
		...
		{
			return (uint)((PositBits >> (int)FractionSizeWithoutSignCheck())
							<< (Size - ExponentSize())
							>> (Size - MaximumExponentSize));
		}
		...
		{
			return (uint)((PositBits >> (int)fractionSize)
							<< (Size - ExponentSize())
							>> (Size - MaximumExponentSize));
		}
		...
		public uint Fraction()
		{
			...
			var bits = IsPositive() ? PositBits : GetTwosComplement(PositBits);
			return (uint)(bits << (int)(Size - fractionSize)
		...
		public uint FractionWithHiddenBit()
		{
			var fractionSize = FractionSize();
			var bits = IsPositive() ? PositBits : GetTwosComplement(PositBits);
			var result = (uint)(bits << (int)(Size - fractionSize)
						 >> (int)(Size - fractionSize));
		...
		public uint FractionWithHiddenBitWithoutSignCheck()
		{
			...
			var result = (uint)(PositBits << (int)(Size - fractionSizeWithoutSignCheck)
		...
		public uint FractionWithHiddenBitWithoutSignCheck(uint fractionSize)
		{
			...
			var result = (uint)(PositBits << numberOfNonFractionBits
		...
		public static Posit32_3 Sqrt(Posit32_3 number)
		{
			...
			var resultExponentBits = (uint)(inputScaleFactor % (1 << MaximumExponentSize));
			if (resultExponentBits < 0)
			{
				resultRegimeKValue -= 1;
				resultExponentBits += 1 << MaximumExponentSize;
			...
			return new Posit32_3(AssemblePositBitsWithRounding(false, resultRegimeKValue, resultExponentBits, resultFractionBits), true);
		}
		...
		public static Posit32_3 operator +(Posit32_3 left, Posit32_3 right)
		{
			...
			return new Posit32_3(AssemblePositBitsWithRounding(resultSignBit, resultRegimeKValue, resultExponentBits, resultFractionBits), true);
		}
