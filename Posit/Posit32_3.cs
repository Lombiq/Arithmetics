...
namespace Lombiq.Arithmetics
{
	public readonly struct Posit32_3 : IComparable, IConvertible, IFormattable, IEquatable<Posit32_3>, IComparable<Posit32_3>
	{
		...
		public static uint  AssemblePositBitsWithRounding(bool signBit, int regimeKValue,ulong exponentBits ,  ulong fractionBits)
		{
			...
			if (exponentShiftedLeftBy < 0) //The exponent is shifted to the right, so no fraction bits will fit in the number
			{
				...
				if (((uint)exponentBits == SignBitMask) && (fractionBits==0)) wholePosit += (uint)(wholePosit & 1); //The first non-fitting exponent bit is 1 (and the others are 0), so we round up if the current last bit is 1. THE FRACTION NEEDS TO BE TESTED HERE TOO!
	...
	#region Methods to handle parts of the Posit 
		...
		public static Posit32_3 operator +(Posit32_3 left, Posit32_3 right)
		{
			...
			return new Posit32_3(AssemblePositBitsWithRounding(resultSignBit, resultRegimeKValue, (uint) resultExponentBits, resultFractionBits), true);
		}
		...
		public static Posit32_3 operator /(Posit32_3 left, Posit32_3 right)
		{
			...
			var fractionSizeChange = PositHelper.GetMostSignificantOnePosition(longResultFractionBits) - (33);
