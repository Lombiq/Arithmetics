...
namespace Lombiq.Arithmetics
{
	public readonly struct Posit8_0 : IComparable, IConvertible, IFormattable, IEquatable<Posit8_0>, IComparable<Posit8_0>
	{
		...
		public uint GetExponentValue()
		{
			...
			exponentMask = (byte)((exponentMask >> (int)FractionSize())
							<< (Size - exponentSize)
		...
		{
			return (byte)((PositBits >> (int)FractionSizeWithoutSignCheck())
							<< (Size - ExponentSize())
							>> (Size - MaximumExponentSize));
		}
		...
		{
			return (byte)((PositBits >> (int)fractionSize)
							<< (Size - ExponentSize())
							>> (Size - MaximumExponentSize));
		}
		...
		public uint Fraction()
		{
			...
			var bits = IsPositive() ? PositBits : GetTwosComplement(PositBits);
			return (byte)(bits << (int)(Size - fractionSize)
		...
		public byte FractionWithHiddenBit()
		{
			var fractionSize = FractionSize();
			var bits = IsPositive() ? PositBits : GetTwosComplement(PositBits);
			var result = (byte)(bits << (int)(Size - fractionSize)
						 >> (int)(Size - fractionSize));
		...
		public byte FractionWithHiddenBitWithoutSignCheck()
		{
			...
			var result = (byte)(PositBits << (int)(Size - fractionSizeWithoutSignCheck)
		...
		public byte FractionWithHiddenBitWithoutSignCheck(uint fractionSize)
		{
			...
			var result = (byte)(PositBits << numberOfNonFractionBits
		...
		public static Posit8_0 Sqrt(Posit8_0 number)
		{
			...
			byte resultFractionBits = 0; //q
			byte startingEstimate = 0; //s0
			byte temporaryEstimate; //t
			byte estimateMaskingBit = (byte)(1 << (int)number.FractionSizeWithoutSignCheck()); //r
			...
			return new Posit8_0(AssemblePositBitsWithRounding(false, resultRegimeKValue, resultExponentBits, resultFractionBits), true);
		}
		...
		public static Posit8_0 operator +(Posit8_0 left, Posit8_0 right)
		{
			...
			return new Posit8_0(AssemblePositBitsWithRounding(resultSignBit, resultRegimeKValue, resultExponentBits, resultFractionBits), true);
		}
		...
		public static Posit8_0 operator /(Posit8_0 left, Posit8_0 right)
		{
			...
			return new Posit8_0(AssemblePositBitsWithRounding(resultSignBit, resultRegimeKValue, (byte)resultExponentBits, resultFractionBits), true);
		}
		...
		public static explicit operator int(Posit8_0 x)
		{
			...
			if (scaleFactor + 1 <= 31) // The posit fits into the range
			{
				...
				{
					result = (byte)(x.FractionWithHiddenBit() <<
