...
namespace Lombiq.Arithmetics
{
	public readonly struct Posit16_4 : IComparable, IConvertible, IFormattable, IEquatable<Posit16_4>, IComparable<Posit16_4>
	{
		...
		public uint GetExponentValue()
		{
			...
			exponentMask = (ushort)((exponentMask >> (int)FractionSize())
							<< (Size - exponentSize)
		...
		{
			return (ushort)((PositBits >> (int)FractionSizeWithoutSignCheck())
							<< (Size - ExponentSize())
							>> (Size - MaximumExponentSize));
		}
		...
		{
			return (ushort)((PositBits >> (int)fractionSize)
							<< (Size - ExponentSize())
							>> (Size - MaximumExponentSize));
		}
		...
		public uint Fraction()
		{
			...
			var bits = IsPositive() ? PositBits : GetTwosComplement(PositBits);
			return (ushort)(bits << (int)(Size - fractionSize)
		...
		public ushort FractionWithHiddenBit()
		{
			var fractionSize = FractionSize();
			var bits = IsPositive() ? PositBits : GetTwosComplement(PositBits);
			var result = (ushort)(bits << (int)(Size - fractionSize)
						 >> (int)(Size - fractionSize));
		...
		public ushort FractionWithHiddenBitWithoutSignCheck()
		{
			...
			var result = (ushort)(PositBits << (int)(Size - fractionSizeWithoutSignCheck)
		...
		public ushort FractionWithHiddenBitWithoutSignCheck(uint fractionSize)
		{
			...
			var result = (ushort)(PositBits << numberOfNonFractionBits
		...
		public static Posit16_4 Sqrt(Posit16_4 number)
		{
			...
			ushort resultFractionBits = 0; //q
			ushort startingEstimate = 0; //s0
			ushort temporaryEstimate; //t
			ushort estimateMaskingBit = (ushort)(1 << (int)number.FractionSizeWithoutSignCheck()); //r
			...
			var resultExponentBits = (ushort)(inputScaleFactor % (1 << MaximumExponentSize));
			if (resultExponentBits < 0)
			{
				resultRegimeKValue -= 1;
				resultExponentBits += 1 << MaximumExponentSize;
			...
			return new Posit16_4(AssemblePositBitsWithRounding(false, resultRegimeKValue, resultExponentBits, resultFractionBits), true);
		}
		...
		public static Posit16_4 operator +(Posit16_4 left, Posit16_4 right)
		{
			...
			{
				resultRegimeKValue -= 1;
				resultExponentBits += (ushort)(1 << MaximumExponentSize);
			}

			return new Posit16_4(AssemblePositBitsWithRounding(resultSignBit, resultRegimeKValue, resultExponentBits, resultFractionBits), true);
		}
		...
		public static Posit16_4 operator *(Posit16_4 left, Posit16_4 right)
		{
			...
			var fractionSizeChange = GetMostSignificantOnePosition(longResultFractionBits) - (leftFractionSize + rightFractionSize + 1);
			var resultFractionBits = (ushort)(longResultFractionBits >> (int)(leftFractionSize + 1 + rightFractionSize + 1 - Size));
			var scaleFactor =
				CalculateScaleFactor(left.GetRegimeKValue(), left.GetExponentValue(), MaximumExponentSize) +
				CalculateScaleFactor(right.GetRegimeKValue(), right.GetExponentValue(), MaximumExponentSize);
		...
		public static Posit16_4 operator /(Posit16_4 left, Posit16_4 right)
		{
			...
			var longResultFractionBits = (uint)(((left.FractionWithHiddenBitWithoutSignCheck()) << (int)(63 - leftFractionSize)) /
		...
		public static explicit operator int(Posit16_4 x)
		{
			...
			if (scaleFactor + 1 <= 31) // The posit fits into the range
			{
				...
				{
					result = (ushort)(x.FractionWithHiddenBit() <<
		...
		public static explicit operator double(Posit16_4 x)
		{
			...
			doubleBits += (longFraction << (Size - GetMostSignificantOnePosition(fraction) - 1)) >> (Size - GetMostSignificantOnePosition(fraction) - 1);
