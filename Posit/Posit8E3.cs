    ...
    public readonly struct Posit8E3 : IComparable, IConvertible, IFormattable, IEquatable<Posit8E3>, IComparable<Posit8E3>
    {
            ...
            ushort fractionBits)
        {
            ...
            if (exponentShiftedLeftBy < 0)
            {
                ...
                if ((byte)exponentBits < SignBitMask)
                {
                    // The first non-fitting exponent bit is zero, so we dont round up.
                    return signBit ? GetTwosComplement(wholePosit) : wholePosit;
                }

                if (((byte)exponentBits == SignBitMask) && (fractionBits == 0))
                {
                    wholePosit += (byte)(wholePosit & 1);
                }
                else
            ...
            if (fractionShiftedLeftBy < 0)
            {
                if (16 + fractionShiftedLeftBy >= 0)
                {
                    fractionBits <<= 16 + fractionShiftedLeftBy;
                }
                else
                {
                    fractionBits >>= -(16 + fractionShiftedLeftBy);
        ...
        public uint GetExponentValueWithoutSignCheck() =>
            (byte)((byte)((PositBits >> (int)FractionSizeWithoutSignCheck())
                << (Size - ExponentSize()))
                >> (Size - MaximumExponentSize));

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public uint GetExponentValueWithoutSignCheck(uint fractionSize) =>
            (byte)((byte)((PositBits >> (int)fractionSize)
                << (Size - ExponentSize()))
                >> (Size - MaximumExponentSize));
        ...
        public static Posit8E3 Abs(Posit8E3 input)
        {
            ...
            return new Posit8E3((byte)((input.PositBits ^ maskOfSignBits) + signBit), fromBitMask: true);
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static byte GetTwosComplement(byte bits) =>
            (byte)(~bits + 1);
