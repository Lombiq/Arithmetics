	...
	public readonly struct Posit8_2 : IComparable, IConvertible, IFormattable, IEquatable<Posit8_2>, IComparable<Posit8_2>
	{
		...
		public static byte  AssemblePositBitsWithRounding(bool signBit, int regimeKValue,ushort exponentBits ,  ushort fractionBits)
		{
			...
			if (exponentShiftedLeftBy < 0) //The exponent is shifted to the right, so no fraction bits will fit in the number
			{
				...
				if (((byte)exponentBits == SignBitMask) && (fractionBits==0)) wholePosit += (byte)(wholePosit & 1); //The first non-fitting exponent bit is 1 (and the others are 0), so we round up if the current last bit is 1. THE FRACTION NEEDS TO BE TESTED HERE TOO!
